...
canvas.width = window.innerWidth > 1080 ? 800 : window.innerWidth - 20;
canvas.height = window.innerHeight > 1080 ? 600 : window.innerHeight - 20;
    ...
    if (y + dy < ballRadius) {
        ...
        if (x > paddleX && x < paddleX + paddleWidth) {
            ...
            if (lives <= 0) {
                ...
                // Reset ball and paddle position, and wait for 5 seconds
                isPaused = true;
                waiting = true;
                setTimeout(() => {
                    x = canvas.width / 2;
        ...
        for (let r = 0; r < brickRowCount; r++) {
            ...
            if (b.status == 1) {
                if (x > b.x && x < b.x + brickWidth && y > b.y && y < b.y + brickHeight) {
                    ...
                    score += 10; // Increment score
                    brickBreakSound.play();
                    setTimeout(() => regenerateBrick(c, r), Math.random() * 60000 + 60000); // Regenerate brick after 1-2.5 minutes
                }
            }
        }
    }
...
function movePaddle() {
    ...
    // Shoot bullets if power up is active
...
document.getElementById('nextStageButton').addEventListener('click', () => {
    currentStage++;
    location.href = `game.html?stage=${currentStage}`;
});
